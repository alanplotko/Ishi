...
// Define keys for AnkiDroid communications
#define KEY_BUTTON          0
...
/*-----------------------------------------
    Communicate deck name to AnkiDroid
-------------------------------------------*/
...
/*-----------------------------------------
    Build deck menu for Pebble app
-------------------------------------------*/
...
/*-----------------------------------------
    Destroy menu tiles after selection
-------------------------------------------*/
...
/*-----------------------------------------------
    Send user feedback on flashcard difficulty
    to AnkiDroid
-------------------------------------------------*/
...
/*-----------------------------------------
    Get number of rows for feedback menu
-------------------------------------------*/
...
/*------------------------------------
    Load feedback menu into window
--------------------------------------*/
...
/*-----------------------------------------
    Remove feedback menu from window
-------------------------------------------*/
...
/*--------------------------------
    Pebble AppMessage handlers
----------------------------------*/

// Receive data from AnkiDroid
...
// Lost data from AnkiDroid
static void inbox_dropped_handler(AppMessageResult reason, void *context)
{
    APP_LOG(APP_LOG_LEVEL_ERROR, "Message dropped!");
}

// Failed to send data to AnkiDroid
static void outbox_failed_handler(DictionaryIterator *iterator, AppMessageResult reason, void *context)
{
    APP_LOG(APP_LOG_LEVEL_ERROR, "Outbox send failed!");
}

// Successfully sent data to AnkiDroid
...
/*------------------------------
    Question window handlers
--------------------------------*/

// Click signifies transition to new stage
...
// Load question text into window
static void question_window_load(Window *window)
{
    ...
    /*  Bind scroll layer to window so that up and down map to scrolling
        Use scroll_layer_set_callbacks to add or override interactivity */
    scroll_layer_set_click_config_onto_window(s_scroll_layer, window);

    // Create and structure text layer
...
// Get number of items in the deck menu
...
/*-----------------------------------------------
    Build the main window for the Pebble app
-------------------------------------------------*/
...
static void main_window_load(Window *window)
{
    ...
    // Ask AnkiDroid for decks
    send(KEY_ACTION, ACTION_DECK_SELECT);
    //sendDeckName(KEY_DECKS, "PEBBLE");
}

/*----------------------------------------------
    Remove layers and menus from main window
------------------------------------------------*/
